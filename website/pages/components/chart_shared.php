...
function writeXAxis($xAxisMin, $xAxisMax, $xAxisStep, $xPad, $yPad) {
	for ($n = $xAxisMin; $n < $xAxisMax; $n += $xAxisStep) {			// omit final number (last point is just for padding)
		...
		// labels
		echo '<text x="' . ($x - 1)  . '%" y="' . (100 - $yPad + 10) . '%" fill="#666666">' . $n . '</text>';
		// markers
		echo '<line x1="' . $x . '%" y1="' . (100 - $yPad) . '%" x2="' . $x . '%" y2="' . (100 - $yPad + 2) . '%" style="stroke:#666666;stroke-width:0.5" />';
	}
	// baseline
	echo '<line x1="' . $xPad . '%" y1="' . (100 - $yPad) . '%" x2="' . (100 - $xPad) . '%" y2="' . (100 - $yPad) . '%" style="stroke:#666666;stroke-width:0.5" />';
}
...
function writeYAxis($yAxisMin, $yAxisMax, $yAxisStep, $xPad, $yPad) {
	for ($n = $yAxisMin; $n <= $yAxisMax; $n += $yAxisStep) {
		...
		// labels
		echo '<text x="' . ($xPad - 1)  . '%" y="' . ($y + 1) . '%" fill="#666666" text-anchor="end">' . $n . '</text>';
		// markers & background lines
		if ($n > $yAxisMin) {
			echo '<line x1="' . ($xPad - 0.6) . '%" y1="' . $y . '%" x2="' . ($xPad) . '%" y2="' . $y . '%" style="stroke:#666666;stroke-width:0.5" />';
			echo '<line x1="' . ($xPad) . '%" y1="' . $y . '%" x2="' . (100 - $xPad) . '%" y2="' . $y . '%" style="stroke:#cccccc;stroke-width:0.5" />';
		}
	}
	// baseline
	echo '<line x1="' . $xPad . '%" y1="' . $yPad . '%" x2="' . $xPad . '%" y2="' . (100 - $yPad) . '%" style="stroke:#666666;stroke-width:0.5" />';
}
...
function drawBar($xVal, $yVal, $xAxisMin, $xAxisMax, $xPad, $yAxisMin, $yAxisMax, $yPad, $colour, $link, $label) {
	...
	if ($xVal >= $xAxisMin && $xVal <= $xAxisMax) {
		if ($link <> '') echo '<a xlink:href="' . $link . '" title="' . $label . '" data-bs-toggle="tooltip">';
		echo '<rect x="' . $x . '%" y="' . $y . '%" width="' . $w . '%" height="' . (100 - $yPad - $y) . '%" style="fill:' . $colour . ';stroke:none;fill-opacity:0.5"><title></title></rect>';
		if ($link <> '') echo '</a>';
	}
}
