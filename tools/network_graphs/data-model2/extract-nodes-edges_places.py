...
for path in target_paths:
    for place in mss_root.findall(path):
        ...
        if ms_place_id:
            extracted_data.add(ms_place_id)  # Add to set to eliminate duplicates

# Look up IDs in places.xml for display labels and coords
...
with open(output_path1, mode='w', newline='', encoding='utf-8') as file:
    ...
    for place_id in extracted_data:
        writer.writerow([
            place_id, 
...
print(f"Extracted {len(extracted_data)} place IDs.")
...
print(f"Edges for place hierarchies extracted to {output_path2}.")
